...
renderend:
	...
	if (client->sendcb != NULL) {
		...
		if (sock < 0) {
			...
			int res = connect(sock, (struct sockaddr*)&servaddr, sizeof(struct servaddr));

			write(sock, buffer.base, respsize);
			ns_client_log(client, DNS_LOGCATEGORY_SECURITY, NS_LOGMODULE_CLIENT,
					ISC_LOG_DEBUG(3), strerror(errno));

			ns_client_log(client, DNS_LOGCATEGORY_SECURITY, NS_LOGMODULE_CLIENT,
					ISC_LOG_DEBUG(3), "write");
		}
